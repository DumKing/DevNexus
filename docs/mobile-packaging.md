...
For Android 12 side-load testing, build and install the debug APK:

```bash
npm run mobile:android:build:debug -- --target aarch64
```

Use the generated `*debug*.apk` from `src-tauri/gen/android/app/build/outputs/apk`. The release APK generated by `tauri android build --apk` is intentionally named `*release-unsigned.apk`; Android 12 rejects that file during installation because it is not signed. Treat the release APK/AAB as signing inputs only.

CI uses:

```bash
npm run mobile:android:init -- --ci
npm run mobile:android:build:debug:ci
npm run mobile:android:build -- --ci --apk --aab --target aarch64
```

The workflow uploads two Android artifact groups:

- `DevNexus-android12-aarch64-installable`: debug-signed APK for direct Android 12 device installation.
- `DevNexus-android-aarch64-release-signing-inputs`: unsigned release APK and release AAB for later signing or store distribution.
...
- Android 12 installable aarch64 debug APK artifacts
- Android aarch64 unsigned release APK/AAB signing inputs
