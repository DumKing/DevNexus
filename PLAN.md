...
- 23:18 修复移动端流水线原生依赖失败：Android/iOS 构建改为 mobile-lite Rust backend，仅注册 dev log 与 LAN Chat 命令；桌面插件模块和 OpenSSL/librdkafka/libcurl 等依赖改为桌面目标专用，移动端前端隐藏未适配插件入口，避免手机包编译和运行时误触桌面工具。
