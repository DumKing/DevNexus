...
- 23:33 改造 iOS 发布产物：移动端流水线 iOS job 从 simulator `.app` 改为真机 `aarch64` 无签名 IPA；第一次尝试通过 Tauri CLI 透传 Xcode 签名参数仍触发 development team 校验，随后改为 init 后直接使用 `xcodebuild` 关闭 code signing 构建 device `.app`，再按 `Payload/<App>.app` 结构打包为 `DevNexus-ios-aarch64-unsigned.ipa`，便于用户下载后自行签名安装。
