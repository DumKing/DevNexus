...
- 23:18 修复移动端流水线原生依赖失败：Android/iOS 构建改为 mobile-lite Rust backend，仅注册 dev log 与 LAN Chat 命令；桌面插件模块和 OpenSSL/librdkafka/libcurl 等依赖改为桌面目标专用，移动端前端隐藏未适配插件入口，避免手机包编译和运行时误触桌面工具；随后将 LAN Chat 附件保存仍需的 rustls `reqwest` 保留为移动端共用依赖。
- 23:33 改造 iOS 发布产物：移动端流水线 iOS job 从 simulator `.app` 改为真机 `aarch64` 无签名 IPA；第一次尝试通过 Tauri CLI 透传 Xcode 签名参数仍触发 development team 校验，随后改为 init 后直接使用 `xcodebuild` 关闭 code signing 构建 device `.app`，再按 `Payload/<App>.app` 结构打包为 `DevNexus-ios-aarch64-unsigned.ipa`，便于用户下载后自行签名安装；补充修正 Xcode 构建配置为 Tauri 生成工程使用的小写 `release`，避免误走 debug 脚本并依赖开发服务器地址文件。
- 23:49 继续修复 iOS unsigned IPA 流水线：确认 Tauri `xcode-script` 依赖 `tauri ios build` 写入并保持存活的 Xcode options server，直接调用 `xcodebuild` 会缺少 `${identifier}-server-addr`；workflow 改为后台启动 `tauri ios build --open` 只做 Tauri 同步和 options server 保活，再执行关闭签名的 `xcodebuild` 并打包 unsigned IPA。
