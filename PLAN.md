...
- 23:33 改造 iOS 发布产物：移动端流水线 iOS job 从 simulator `.app` 改为真机 `aarch64` 无签名 IPA，CI 禁用 Xcode code signing 后将生成的 device `.app` 按 `Payload/<App>.app` 结构打包为 `DevNexus-ios-aarch64-unsigned.ipa`，便于用户下载后自行签名安装。
